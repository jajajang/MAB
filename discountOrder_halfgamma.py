    ...
    for D in delty:
        for factor in factory:
            ...
            #                        2 * np.log(t) / np.sum(action_list[i, -w:]))
                ...
                file1name=f'halfgamma_Order_T_{T}_alpha_{alpha}_ptop_{p_top}_pbot_{p_bot}_delta_{D}_factor_{factor}_discount_result_only.csv'
